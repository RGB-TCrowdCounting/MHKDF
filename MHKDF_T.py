...
def load_pretrain_T(net):
    pretrain_path = r'mit_b2.pth'
...
class MHKDF_T(nn.Module):
    def __init__(self, pretrained=True):
        super(MHKDF_T, self).__init__()
        self.upsample_add = upsample_add
        self.upsample = cus_sample
        seg_r = mit_b2()
        seg_r = load_pretrain_T(seg_r)
        seg_d = mit_b2_d()  ##
        seg_d = load_pretrain_T(seg_d)

        self.layersR = seg_r
        self.layersD = seg_d

        self.trans8 = nn.Conv2d(512, 64, 1)  ##b3 64,128,320,512#############b0 32, 64, 160, 256########mit_b3
        self.trans4 = nn.Conv2d(320, 64, 1)
        ...
        self.selfdc_8 = MSIM_T(64, 64)
        self.selfdc_4 = MSIM_T(64, 64)
    ...
    def forward(self, RGBT):
        ...
        in_data_4_d = self.trans4(dataD_4)

        in_data_2_r = self.trans2(dataR_2)
        in_data_2_d = self.trans2(dataD_2)
        in_data_1_r = self.trans1(dataR_1)

        in_data_2_aux = self.t_trans2(dataD_2, dataR_2)# +self.upconv22(in_data_2_dd)##self.t_trans2(dataD_2, dataR_2)
        in_data_4_aux = self.t_trans4(dataD_4, dataR_4)
        in_data_8_aux = in_data_8_r
